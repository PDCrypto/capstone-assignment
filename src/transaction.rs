...
// method
// f. create a transaction_start_date variable by passing the fourth item of lines into
// NaiveDate::parse_from_str function followed by an unwrap method
// g. create a transaction_end_date variable by passing the fifth item of lines into
// NaiveDate::parse_from_str function followed by an unwrap method
...
// country_to_continent functionNaiveDate::parse_from_str function followed by an
// unwrap method
...
impl Transaction {
  pub fn from_csv_line(line: &str) -> Result<Transaction, String> {
    ...
    if fields.len() != 7 {
      return Err("CSV File got problem: Wrong number of columns".to_owned())
    }
